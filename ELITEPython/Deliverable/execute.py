from SchedulerV000 import run_opt, run_bf, run_wf
...
def make_df(dict):
        ...
        df = pd.DataFrame.from_dict(dict, orient='index', columns=all_cols)
        #df['ReasonId'] = 100
        return df
...
def main():
        ...
        for value in test:
                ...
                if value == 'WF':
                        timer0 = time.monotonic()
                        best_result, worst_result, best_sched, worst_sched = run_wf(start_time, end_time, historical_down_periods_file, failure_rate_file, 
                                                                                product_related_characteristics_file, energy_price_file, job_info_file,
                                                                                scenario)
                        timer1 = time.monotonic()
                        elapsed_time = timer1-timer0
