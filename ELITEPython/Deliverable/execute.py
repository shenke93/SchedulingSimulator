...
def executeSuddenBreakdowns():
        print_ul('Scheduler v0.0.0 for sudden breakdowns')
        ...
        export_folder = os.getcwd() + configParser.get('output-config', 'export_folder') + strftime("%Y%m%d_%H%M", localtime())
        os.makedirs(export_folder, exist_ok=True)
        output_init_breakdowns = os.path.join(export_folder, configParser.get('output-config', 'output_init_breakdowns'))
        output_final_breakdowns = os.path.join(export_folder, configParser.get('output-config', 'output_final_breakdowns'))
        interactive = configParser.getboolean('output-config', 'interactive')
        export = configParser.getboolean('output-config', 'export')
        ...
        for value in test:
                if value == 'GA':
                        ...
                        # output files to csv's
                        orig.to_csv(output_init_breakdowns)
                        best.to_csv(output_final_breakdowns)
...
#                         show_energy_plot(best, energy_price, prod_char, 'Best schedule (GA) ({:} gen)'.format(gen), namecolor, downtimes=downtimes)
                        if export is True:
                                print('Export to', export_folder)
#                                 plt.savefig(os.path.join(export_folder, r"best_sched.png"), dpi=300)
                                save_energy_plot(best, energy_price, prod_char, name='Breakdown_Best', folder=export_folder, title='Best schedule (GA) ({:} gen)'.format(gen), colors=namecolor, downtimes=downtimes)
