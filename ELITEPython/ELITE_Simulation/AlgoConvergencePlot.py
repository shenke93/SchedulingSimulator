...
#     DNA_SIZE = len(job_dict_new)
    ...
    GAResult_dict.update({0:get_energy_cost(original_schedule, first_start_time, job_dict_new, price_dict_new)})
    
    for i in range(1, N_GENERATIONS+1):
#         start_stamp = time.time() 
        # Running GA methond here
        ga = run_GA(dna_size=len(waiting_jobs), cross_rate=CROSS_RATE, mutation_rate=MUTATION_RATE, pop_size=POP_SIZE,
                    pop=waiting_jobs, n_generations=i, start_time=first_start_time, job_dict=job_dict_new,
                    price_dict=price_dict_new)
        
#         end_stamp = time.time()
        
        print("Optimal cost:", ga)
#         print("Optimal schedule:", ga.values())
#         print("Time consumption:", end_stamp-start_stamp)
        GAResult_dict.update({i:ga})
    
    # write the result to csv for plot
    with open('ga_convergence_plot.csv', 'w', newline='\n') as csv_file:
