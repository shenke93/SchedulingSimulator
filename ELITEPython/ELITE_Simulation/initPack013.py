...
#         with open('jobInfoPack_ga_012.csv', 'w', encoding='utf-8') as csvOutput:
...
try:
    with open('productPack.csv', 'r', encoding='utf-8') as csvInput:
        with open('productPack_ga_013.csv', 'w', encoding='utf-8') as csvOutput:
            writer = csv.writer(csvOutput, lineterminator='\n')
            reader = csv.reader(csvInput)
            ...
            for row in reader:
                row.append(round(np.random.uniform(1, 2), 2))
                all.append(row)
            writer.writerows(all)
except:
