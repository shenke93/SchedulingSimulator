...
class Activity(object):
    ...
    def __init__(self, idx, idx_project, release_time, required_duration, consumption_mode1, consumption_mode2):
    ...
    def __str__(self):
        res = "idx: %d \n idx_project: %d \n release_time: %d \n required_duration: %d \n consumption_mode1: %d \n consumption_mode2: %d" % \
         (self.idx, self.idx_project, self.release_time, self.required_duration, self.consumption_mode1, self.consumption_mode2)
    ...
    try:
        with open(jobInfoFile, encoding='utf-8') as jobInfo_csv:
            ...
            for row in reader:
                ...
                required_duration = int(row['RequiredDuration'])
                consumption_mode1 = int(row['ResourceMode1'])
                consumption_mode2 = int(row['ResourceMode2'])
               
                # Create job instances
                job = Activity(idx, idx_project, release_time, required_duration, consumption_mode1, consumption_mode2)
