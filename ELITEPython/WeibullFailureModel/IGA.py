...
def weibull_failure(t, alpha, beta):
    return 1 - weibull_reliability(t, alpha, beta)


def weibull_reliability(t, alpha, beta):
    ''' Weibull reliability function in time R(t)'''
    return np.exp(-(t/alpha)**beta)


def weibull_pdf(t, alpha, beta): # Weibull PDF
    ''' Weibull PDF in time f(t)'''
    t = t+0.1
...
def read_failure(slope, intercept, price_dict):
    ''' 
    Generate hourly dependent failure rate.

    Parameters
    ----------
    slope : float
        Attribute used to estimate a weibull parameter. 
    intercept : float
        Attribute used to estimate a weibull parameter.
    price_dict : dict
    ...
    Returns
    -------
    Dictionary containing failure rate of each hour.
    '''
    beta = slope
    alpha = np.exp(-(intercept/beta))
    health_dict = {}
    for key in price_dict:
        sat_offset = (key.weekday() - 5) % 7  
        h = key.hour
        health_dict.update({key:weibull_pdf(h+sat_offset*24, alpha, beta)}) 
    return health_dict   
...
#     end_time = datetime(2017, 11, 15, 0, 0)
    ...
    failure_dict_new = read_failure(0.72, -2.86, price_dict_new)
    
  
    # write corresponding failure dict into file
    with open('ga_013_failure_dict.csv', 'w', newline='\n') as csv_file:
